...
#define SPEED   130 //motor in   speed
...
void auto_following(){
 ...
 { 
  //both sensor detected obstacle, go ahead
      //go_Back();  //call go-back instead ofgo-advance as the senor is mounted at the back. 
      go_Advance();  // go back from the obstacle until the sensor no longer detects the obstacle;
                    // the senors have been configured to be working at  6cms using the potentiometer
        
 }
 else  if (IRvalueLeft==HIGH && IRvalueRight==HIGH)
 {
     alarm(); //beeps when it stops moving
